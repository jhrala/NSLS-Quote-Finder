...
if ((Test-Path (Join-Path $CommandsDir "find-quotes.md")) -or (Test-Path (Join-Path $CommandsDir "update-quotes.md"))) {
    Write-Host "  ⚠️  Existing quote search commands found. They will be overwritten." -ForegroundColor Yellow
    Write-Host ""
}
...
# Use the Windows path format (backslashes) for the Read tool in Claude Code
...
Write-Host "  ✓ Installed /find-quotes" -ForegroundColor Green
...
Write-Host "  ✓ Installed /update-quotes" -ForegroundColor Green
...
if (-not (Test-Path $QuotesDb)) {
    ...
    Write-Host "  ✓ Created empty quotes database" -ForegroundColor Green
} else {
    try {
        $quotes = Get-Content $QuotesDb -Raw | ConvertFrom-Json
        $count = $quotes.Count
        Write-Host "  ✓ Quotes database already exists ($count quotes)" -ForegroundColor Green
    } catch {
        Write-Host "  ✓ Quotes database already exists" -ForegroundColor Green
    }
}
